...
fn main() -> Result<()> {
    let matches = Command::new("ai_library_sys")
        .about("图书馆管理系统")
        .subcommand(Command::new("add").alias("a").alias("A").about("添加新书"))
        .subcommand(
            Command::new("remove").about("删除书籍").arg(
                Arg::new("isbn")
                    .short('i')
                    .long("isbn")
                    .help("书籍ISBN")
        ...
        .subcommand(Command::new("list").about("列出所有书籍"))
        .subcommand(
            Command::new("borrow").about("借阅书籍").arg(
                Arg::new("isbn")
                    .short('i')
                    .long("isbn")
                    .help("书籍ISBN")
        ...
        .subcommand(
            Command::new("return").about("归还书籍").arg(
                Arg::new("isbn")
                    .short('i')
                    .long("isbn")
                    .help("书籍ISBN")
    ...
    match matches.subcommand() {
        ...
        Some(("remove", sub_matches)) => {
            let isbn = if let Some(isbn) = sub_matches.get_one::<String>("isbn") {
                isbn.clone()
            } else {
                Input::new()
                    .with_prompt("请输入要删除的书籍ISBN")
                    .interact_text()?
            };
        ...
        Some(("borrow", sub_matches)) => {
            let isbn = if let Some(isbn) = sub_matches.get_one::<String>("isbn") {
                isbn.clone()
            } else {
                Input::new()
                    .with_prompt("请输入要借阅的书籍ISBN")
                    .interact_text()?
            };
        ...
        Some(("return", sub_matches)) => {
            let isbn = if let Some(isbn) = sub_matches.get_one::<String>("isbn") {
                isbn.clone()
            } else {
                Input::new()
                    .with_prompt("请输入要归还的书籍ISBN")
                    .interact_text()?
            };
